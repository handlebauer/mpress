...
import { LONG_PRESS_MAX_LENGTH } from '~/constants'
...
export default function LongPress(props: LongPressProps) {
  ...
  const progress = (value.length / LONG_PRESS_MAX_LENGTH) * 100
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      if (event.key === 'Backspace') {
        ...
        setValue(prev => (prev + event.key).slice(0, LONG_PRESS_MAX_LENGTH))
      }
    }
